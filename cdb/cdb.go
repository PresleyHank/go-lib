...
func verifyChecksum(f *os.File, path string) error {
    st, err := f.Stat()
	if err != nil { return fmt.Errorf("can't stat %s: %s", path, err) }

    sz := st.Size()
    if sz < (2048+sha256.Size) { return fmt.Errorf("cdb file %s too small", path) }
	...
	if err != nil { return fmt.Errorf("can't seek %s: %s", path, err) }

    var eck [sha256.Size]byte

    n, err := f.Read(eck[:])
    if err != nil { return fmt.Errorf("can't read checksum of %s: %s", path, err) }

    if n != sha256.Size { return fmt.Errorf("i/o error while reading checksum of %s: only read %d bytes", n) }

    // Verify checksum now
    hh := sha256.New()
    err = util.MmapReader(f, 0, datasz, hh)
    if err != nil { return fmt.Errorf("i/o error during checksum calculation: %s", err) }
