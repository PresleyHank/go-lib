...
func (db *PackageDB) GetListByUid(uid uint32) ([]*Pkg) {
    ...
    if r, ok := db.byUid[uid]; ok { return r }
...
func (db *PackageDB) GetByName(nm string) *Pkg {
    ...
    if r, ok := db.byName[nm]; ok { return r }
    return nil
}
...
func (db *PackageDB) GetByUid(uid uint32) *Pkg {
    ...
    if r, ok := db.byUid[uid]; ok { return r[0] }
    return nil
}
...
func exists(fn string) bool {
    ...
    if !st.Mode().IsRegular() { return false }
...
func parseXML(fn string) ([]*Pkg, error) {
    ...
    if err != nil { return nil, fmt.Errorf("Cannot parse %s: %s", fn, err) }
    ...
    for i, x := range v.Pkgs {
        ...
        if len(x.Certstr.Cert) > 0 {
            ...
            if err != nil { return nil, fmt.Errorf("%s: Can't decode cert hex: %s", x.Name, err) }

            if len(b) > 0 {
                crt, err := x509.ParseCertificate(b[:])
                if err != nil { return nil, fmt.Errorf("%s: Can't parse X509 DER cert: %s", x.Name, err) }
